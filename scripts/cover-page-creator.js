...
function initPdfExtractor() {
    ...
    function sampleBackgroundColor(imageData, x, y, boundsX0, boundsY0, boundsX1, boundsY1) {
        ...
        // Define sampling offsets (pixels away from current position)
        const offsets = [
        ...
        for (const {dx, dy} of offsets) {
            ...
            if (sx >= boundsX0 && sx < boundsX1 && sy >= boundsY0 && sy < boundsY1) {
                ...
                // Add sample if it's likely background (brighter)
                const brightness = (r + g + b) / 3;
                if (brightness > 200) {
                    samples.push({r, g, b});
                }
        ...
        // If we have samples, calculate the average color
        if (samples.length > 0) {
        ...
        // Default to white if no good samples
        return {r: 255, g: 255, b: 255};
    }
    ...
    function detectColoredTextRegions(imageData) {
        ...
        // Copy image data to temporary canvas
        for (let i = 0; i < data.length; i++) {
            tempImageData.data[i] = data[i];
        }
        
        // Apply edge detection to find text-like contours
        applyEdgeDetection(tempImageData);
        tempCtx.putImageData(tempImageData, 0, 0);
        
        // Scan for horizontal clusters of edges (likely text)
        const visited = new Uint8Array(width * height);
        
        // Scan with wider spacing for efficiency
        const scanStep = 5;
        ...
        for (let y = 0; y < height; y += scanStep) {
            ...
            for (let x = 0; x < width; x += 2) {
                ...
                // Check if this is an edge pixel (part of potential text)
                if (brightness < 200 && !visited[y * width + x]) {
                    // If we're not already in a line, mark the start
                    if (!isInLine) {
                        lineStart = x;
                        isInLine = true;
                        
                        // Estimate the height of this text line
                    ...
                    // Only consider lines that are reasonably long (likely text, not noise)
                    if (x - lineStart > 20) {
            ...
            // Handle line that extends to the right edge
            if (isInLine) {
                if (width - lineStart > 20) {
                    const padding = Math.max(5, Math.floor(lineHeight * 0.5));
    ...
    function applyEdgeDetection(imageData) {
        ...
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                ...
                // If there's a significant change in either direction, mark as edge
    ...
    function estimateTextLineHeight(imageData, startX, startY, width, height) {
        ...
        for (let y = startY + 1; y < Math.min(height - 1, startY + 50) && !bottomFound; y++) {
            ...
            if (hasEdge) {
                ...
                // If we've gone a few rows without finding edges, consider it the bottom
                if (y > lineBottom + 5) {
                    bottomFound = true;
                }
            }
        ...
        // Search upward for the top of the line
        let topFound = false;
        let lineTop = startY;
        ...
        for (let y = startY - 1; y >= Math.max(0, startY - 50) && !topFound; y--) {
            ...
            if (hasEdge) {
                ...
                // If we've gone a few rows without finding edges, consider it the top
                if (y < lineTop - 5) {
                    topFound = true;
                }
            }
    ...
    function mergeOverlappingRegions(regions) {
        ...
        // Make a copy to avoid modifying the original array while iterating
    ...
    function addColoredTextRegionsToTextContent(textContent, regions, viewport, scale) {
        // Check if regions might overlap with existing text items
        for (const region of regions) {
            let overlapsExisting = false;
            
            // Check against existing text items
            for (const item of textContent.items) {
                const transform = item.transform;
                const fontSize = Math.abs(transform[3]);
                ...
                const textX1 = itemX + textWidth
                /**
     * Perform additional text removal to catch text that might have been missed
     * @param {ImageData} imageData - Canvas image data
     */
    function performAdditionalTextRemoval(imageData) {
        const { width, height, data } = imageData;
        
        // Focus on regions likely to contain colored text
        // 1. Look for small clusters of non-white, non-graphics pixels
        // 2. Detect horizontal/vertical edges that are likely text
        ...
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                ...
                // Look for significant changes that might indicate text edges
                // Focus on sharp transitions in colored areas
                if (diffRight > 50 || diffDown > 50) {
                    // Check if this is likely text (not part of a major graphic element)
                    const brightness = (r + g + b) / 3;
                    const isColored = Math.max(r, g, b) - Math.min(r, g, b) > 30;
                    
                    if (isColored && brightness < 220) {
                        tempData[idx] = 255;     // Mark as edge
                        tempData[idx + 1] = 255;
                        tempData[idx + 2] = 255;
                        tempData[idx + 3] = 255;
                    } else {
        ...
        // Now process the detected edges to remove text-like content
        const visited = new Uint8Array(width * height);
        ...
        for (let y = 0; y < height; y += 3) {
            for (let x = 0; x < width; x += 3) {
                ...
                // If this is an edge pixel that hasn't been visited
                if (tempData[idx] > 0 && !visited[y * width + x]) {
                    // Flood fill to find the connected component
                    const component = floodFillEdges(tempData, visited, x, y, width, height);
                    
                    // If the component is a suitable size for text
                    if (component.pixels.length > 5 && component.pixels.length < 500 &&
                        component.width > 3 && component.height > 3 &&
                        component.width < 200 && component.height < 50) {
                        
                        // Sample the surrounding area to determine the background color
                        const bgColor = sampleBackgroundForComponent(data, component, width, height);
                        
                        // Replace the component with the background color
     ...
     * @param {Uint8ClampedArray} edgeData - Edge detection data
     ...
     */
    function floodFillEdges(edgeData, visited, startX, startY, width, height) {
    ...
    /**
     * Sample the background color for a component
     ...
     * @returns {Object} - Background color {r, g, b}
     */
    function sampleBackgroundForComponent(data, component, width, height) {
        // Expand the bounding box slightly
        const padding = Math.max(5, Math.floor(component.height * 0.2));
        
        const x0 = Math.max(0, component.minX - padding);
        const y0 = Math.max(0, component.minY - padding);
        const x1 = Math.min(width - 1, component.maxX + padding);
        const y1 = Math.min(height - 1, component.maxY + padding);
        
        // Create a set of component pixels for fast lookup
        const componentPixels = new Set();
        for (const {x, y} of component.pixels) {
            componentPixels.add(y * width + x);
        }
        
        // Sample pixels around the component's bounding box
        const samples = [];
        
        // Sample top and bottom edges
        for (let x = x0; x <= x1; x += 2) {
            // Top edge
            for (let y = y0; y < y0 + padding && y < component.minY; y++) {
            ...
            // Bottom edge
            for (let y = component.maxY + 1; y <= y1; y++) {
                if (!componentPixels.has(y * width + x)) {
                    const idx = (y * width + x) * 4;
                    samples.push({
                        r: data[idx],
        ...
        for (let y = y0; y <= y1; y += 2) {
            // Left edge
            for (let x = x0; x < x0 + padding && x < component.minX; x++) {
                if (!componentPixels.has(y * width + x)) {
                    const idx = (y * width + x) * 4;
                    samples.push({
                        r: data[idx],
            ...
            // Right edge
            for (let x = component.maxX + 1; x <= x1; x++) {
                if (!componentPixels.has(y * width + x)) {
                    const idx = (y * width + x) * 4;
                    samples.push({
                        r: data[idx],
        ...
        // If we have samples, group them by color
        if (samples.length > 0) {
            // Group by similar colors
            const colorGroups = groupSimilarColors(samples);
            ...
            for (let i = 0; i < colorGroups.length; i++) {
                if (colorGroups[i].length > maxCount) {
                    maxCount = colorGroups[i].length;
                    maxGroup = i;
                }
            }
            
            // Use the average of the largest group
            if (colorGroups[maxGroup] && colorGroups[maxGroup].length > 0) {
                const avgColor = colorGroups[maxGroup].reduce((acc, sample) => {
                    acc.r += sample.r;
                    acc.g += sample.g;
                    acc.b += sample.b;
                    return acc;
                ...
                return {
                    r: Math.round(avgColor.r / colorGroups[maxGroup].length),
                    g: Math.round(avgColor.g / colorGroups[maxGroup].length),
                    b: Math.round(avgColor.b / colorGroups[maxGroup].length)
                };
            }
        }
    ...
    /**
     * Group color samples by similarity
     * @param {Array} samples - Color samples
     * @returns {Array} - Groups of similar colors
     */
    function groupSimilarColors(samples) {
        const groups = [];
        const SIMILARITY_THRESHOLD = 30; // Adjust as needed
            ...
            for (const group of groups) {
                if (group.length > 0) {
                    ...
                    const distance = Math.sqrt(
                        Math.pow(sample.r - representative.r, 2) +
                        Math.pow(sample.g - representative.g, 2) +
                        Math.pow(sample.b - representative.b, 2)
                    );
...
function initPdfExtractor() {
    ...
    function addColoredTextRegionsToTextContent(textContent, regions, viewport, scale) {
        ...
        for (const region of regions) {
            ...
            // If it doesn't overlap with existing items, add it as a new text item
            if (!overlapsExisting) {
                // Create a synthetic text item for the region
                // Note: We don't know the actual text content, but we're just
                // interested in the region for masking purposes
                ...
                textContent.items.push({
                    str: "~COLORED_TEXT~", // Placeholder text
                    dir: "ltr",
                    ...
                    fontName: "g_d0_f1" // Placeholder font name
                });
            }
        }
    }
