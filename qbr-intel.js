...
const fbCompany       = document.getElementById('fb-company');
...
const CUSTOMERS = {
  abc: {
    ...
    mrr: '$3,840', since: 'May 2021', health: '82', healthColor: '#2d6a2d', am: 'Sarah Johnson',
  ...
  river: {
    ...
    mrr: '$2,100', since: 'Jan 2022', health: '71', healthColor: '#7a4f0a', am: 'Mark Davies',
  ...
  peak: {
    ...
    mrr: '$5,200', since: 'Mar 2020', health: '91', healthColor: '#2d6a2d', am: 'Lisa Tran',
...
async function generateAIBrief() {
  ...
  const signalLines = c.signals.map(s => `- ${s.label}: ${s.title} (${s.src})`).join('\n');
...
Write my QBR conversation brief.`;
  ...
  try {
    ...
    aiText.textContent = 'Could not reach the AI service. Make sure the Netlify function is deployed and ANTHROPIC_API_KEY is set.';
  }
...
async function submitFeedback() {
  ...
  const company  = fbCompany.value.trim();

  if (!actions.length && !sources.length && !freeform && !selectedRating) {
    fbStatus.textContent = 'Please fill in at least one field before submitting.';
  ...
  const payload = {
    ...
    viewedCustomer: customerSelect.options[customerSelect.selectedIndex].text,
  ...
  try {
    ...
    if (data.ok) {
      ...
      fbStatus.textContent = 'Error: ' + (data.error || 'Could not send feedback.');
      fbStatus.className   = 'fb-status err';
    }
  } catch {
    fbStatus.textContent = 'Network error — could not reach the server.';
    fbStatus.className   = 'fb-status err';
  }
...
customerSelect.addEventListener('change', e => renderCustomer(e.target.value));
